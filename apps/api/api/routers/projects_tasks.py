...
from api.models import Project, ProjectMember, Task, TaskAssignment, TaskStatus, User
...
from api.serializers import task_to_schema
...
async def list_projects(
    ...
    rows = (await db.execute(select(Project).order_by(Project.created_at.desc()).offset(offset).limit(limit))).scalars().all()
...
async def create_project(
    ...
    await write_audit(db, action="project_created", entity_type="project", entity_id=project.id, user_id=current_user.id)
...
async def list_tasks(
    ...
    rows = (await db.execute(query.order_by(Task.created_at.desc()).offset(offset).limit(limit))).scalars().all()
    visible = [t for t in rows if can_view_task(current_user, t)]
    return PaginatedResponse(
        data=[task_to_schema(t) for t in visible],
        pagination=Pagination(limit=limit, offset=offset, total=len(visible), has_more=offset + limit < total),
    )
...
async def create_task(
    ...
    await write_audit(db, action="task_created", entity_type="task", entity_id=task.id, user_id=current_user.id)
...
async def verify_task(
    ...
    await event_bus.publish(
        ...
        task_id=task.id,
        user_id=current_user.id,
        data={"taskId": task.id, "from": old_status, "to": task.status, "verification": body.result},
    )
    return SuccessResponse(
        data=VerificationOut(
            id=vr.id,
